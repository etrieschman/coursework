...
plt.axhline(
    y=g_fs[np.argmin(np.abs(final_grades - target))], color="C2", linestyle="--"
)
plt.show()
...
w_h, w_l, w_n, w_m, w_f, w_p = 0.15, 0.15, 0.05, 0.23, 0.37, 0.05
...
plt.axvline(x=target, color="C1", linestyle="--")
plt.axhline(y=get_g_f(target), color="C2", linestyle="--")
plt.xlabel("Course grade")
plt.ylabel("grade on final")
plt.show()
